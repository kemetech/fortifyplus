...
Route::group(['middleware' => config('fortifyplus.middleware', ['web'])], function () {
    ...
    if (Features::enabled(Features::emailVerification())) {
        if ($enableViews) {
            Route::get(RoutePath::for('verification.notice', '/email/verify'), [EmailVerificationPromptController::class, '__invoke'])
                ->middleware([config('fortify.auth_middleware', 'auth').':'.config('fortifyplus.guard.user')])
                ->name('verification.notice');

            Route::get(RoutePath::forAdmin('verification.notice', '/email/verify'), [EmailVerificationPromptController::class, '__invoke'])
                ->middleware([config('fortify.auth_middleware', 'auth').':'.config('fortifyplus.guard.admin')])
                ->name('admin.verification.notice');
        }

        Route::get(RoutePath::for('verification.verify', '/email/verify/{id}/{hash}'), [VerifyEmailController::class, '__invoke'])
            ->middleware([config('fortify.auth_middleware', 'auth').':'.config('fortify.guard.user'), 'signed', 'throttle:'.$verificationLimiter])
            ->name('verification.verify');

        Route::post(RoutePath::for('verification.send', '/email/verification-notification'), [EmailVerificationNotificationController::class, 'store'])
            ->middleware([config('fortify.auth_middleware', 'auth').':'.config('fortify.guard.user'), 'throttle:'.$verificationLimiter])
        ...
        Route::get(RoutePath::forAdmin('verification.verify', '/admin/email/verify/{id}/{hash}'), [AdminVerifyEmailController::class, '__invoke'])
            ->middleware([config('fortifyplus.auth_middleware', 'auth').':'.config('fortify.guard.admin'), 'signed', 'throttle:'.$verificationLimiter])
        ...
        Route::post(RoutePath::forAdmin('verification.send', '/admin/email/verification-notification'), [EmailVerificationNotificationController::class, 'store'])
            ->middleware([config('fortifyplus.auth_middleware', 'auth').':'.config('fortifyplus.guard.admin'), 'throttle:'.$verificationLimiter])
    ...
    if (Features::enabled(Features::twoFactorAuthentication())) {
        ...
        $twoFactorMiddleware = Features::optionEnabled(Features::twoFactorAuthentication(), 'confirmPassword')
            ? [config('fortify.auth_middleware', 'auth').':'.config('fortifyplus.guard.user'), 'password.confirm']
            : [config('fortify.auth_middleware', 'auth').':'.config('fortifyplus.guard.user')];

        $adminTwoFactorMiddleware = Features::optionEnabled(Features::twoFactorAuthentication(), 'confirmPassword')
            ? [config('fortify.auth_middleware', 'auth').':'.config('fortifyplus.guard.admin'), 'password.confirm']
            : [config('fortify.auth_middleware', 'auth').':'.config('fortifyplus.guard.admin')];
